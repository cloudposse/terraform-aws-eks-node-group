...
locals {
  ...
  ng = {
    ...
    subnet_ids = sort(local.placement_availability_zone == null ? var.subnet_ids : toset([for subnet in data.aws_subnet.default : subnet.id if subnet.availability_zone == local.placement_availability_zone]))
