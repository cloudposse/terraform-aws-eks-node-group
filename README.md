...
| <a name="input_force_node_group_replacement"></a> [force\_node\_group\_replacement](#input\_force\_node\_group\_replacement) | Force Node Group replacement when upgrading to a new Kubernetes version. If set to `true`, the Node Groups will be replaced instead of updated in-place | `bool` | `false` | no |
