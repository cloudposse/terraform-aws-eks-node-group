...
| <a name="input_force_update_version"></a> [force\_update\_version](#input\_force\_update\_version) | Force Kubernetes version update if existing Pods are unable to be drained from the nodes due to PodDisruptionBudget or taint/toleration issues | `bool` | `false` | no |
...
| <a name="input_replace_node_group_on_version_update"></a> [replace\_node\_group\_on\_version\_update](#input\_replace\_node\_group\_on\_version\_update) | Force Node Group replacement when updating to a new Kubernetes version. If set to `true`, the Node Groups will be replaced instead of updated in-place | `bool` | `false` | no |
