...
| <a name="input_launch_template_disk_device_name"></a> [launch\_template\_disk\_device\_name](#input\_launch\_template\_disk\_device\_name) | The name of the instance storage device to which the volume created by the launch template gets attached to. | `string` | `"/dev/xvda"` | no |
