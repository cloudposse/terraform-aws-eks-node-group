...
locals {
  ...
  kubelet_extra_args = join(" ", compact([local.kubelet_label_args, var.kubelet_additional_options]))
  ...
  need_bootstrap = local.enabled ? length(compact([var.kubelet_additional_options,
