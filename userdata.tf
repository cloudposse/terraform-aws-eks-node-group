...
locals {
  ...
  userdata_vars = {
    ...
    kubelet_flags                   = length(var.kubelet_additional_options) == 0 ? "" : <<-EOF
      kubelet:
          flags: ${local.kubelet_extra_args}
    EOF
  }
  ...
  cluster_data = {
    ...
    cluster_cidr               = var.cluster_cidr == null ? data.aws_vpc.this[0].cidr_block_associations[0].cidr_block : var.cluster_cidr
  }
